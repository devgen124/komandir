jQuery(function ($) {
	...
	function cityToSelect($citybox, current_cities) {
		...
		if ($citybox.is('input')) {
			...
			var input_id = $citybox.attr('id');
			var placeholder = $citybox.attr('placeholder');
			var className = $citybox.attr('class');

			$citybox.replaceWith('<select name="' + input_name + '" id="' + input_id + '" class="city_select ' + className + '" placeholder="' + placeholder + '"></select>');
		...
		for (var index in current_cities) {
			if (current_cities.hasOwnProperty(index)) {
				...
				options = options + '<option value="' + cityName + '" ' + selected + '>' + cityName + '</option>';
			}
		}
