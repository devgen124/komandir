...
class VariationOffer
{
    ...
    public static function process(\SimpleXMLElement $element, $productGuid, $rate)
    {
        ...
        */
        if ($offerIsRemoved) {
            Logger::log('(variation) offer is market for deletion', [$variationID, (string) $element->Ид]);
        ...
        if (!$offerIsRemoved && VariationOfferPrices::offerHasPriceData($element)) {
        ...
        if (!$offerIsRemoved && OfferStocks::offerHasStockData($element)) {
            if (!\apply_filters('itglx_wc1c_ignore_offer_set_stock_data', false, $variationID, $productID)) {
                OfferStocks::set($variationID, OfferStocks::resolve($element), $productID);
            } else {
                Logger::log(
