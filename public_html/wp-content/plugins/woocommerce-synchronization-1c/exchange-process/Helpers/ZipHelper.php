...
class ZipHelper
{
    ...
    public static function extract($filename)
    {
        ...
        for ($i = $_SESSION['zipExtractedEntries']; $i < $zip->numFiles; ++$i) {
            ...
            if ($result !== true) {
                throw new \Exception('[zip] failed extract entry ' . $zip->getNameIndex($i) . ', to - ' . dirname($filename));
            }
    ...
    private static function start($filename)
    {
        ...
        if ($result !== true) {
            throw new \Exception('[zip] failed open archive ' . $filename . ', code - ' . $result);
        }
    ...
    private static function end($zip, $filename, $unlink = true)
    {
        ...
        if ($zip->close() !== true) {
            throw new \Exception('[zip] failed close archive ' . $filename);
        }
