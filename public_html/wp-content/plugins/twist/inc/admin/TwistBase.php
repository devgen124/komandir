...
if(!class_exists("TwistBase")) {
	class TwistBase {
    	...
    	private $server_host = "https://www.codeixer.com/wp-json/el-api/";
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=twist_fupc' ) . "'>Check Update</a>";
							}
							return $links;
						}, 10, 2 );
        ...
        public function updateMessageCB($data, $response){
            ...
            if(isset($data->package) && empty($data->package)) {              
                if(empty($data->update_denied_type)) {
                    print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'> You are using an unregistered version of <strong>".WPGS_NAME."</strong> and are not receiving compatibility and security updates. <a href='".admin_url('admin.php?page=codeixer-dashboard')."'>Register Now</a></span>";
                }elseif($data->update_denied_type=="L"){
                    print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'> You are using an unregistered version of <strong>".WPGS_NAME."</strong> and are not receiving compatibility and security updates. <a href='".admin_url('admin.php?page=codeixer-dashboard')."'>Register Now</a></span>";
                }elseif($data->update_denied_type=="S"){
                    print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Your license has been expired. Please <a target='_blank' href='https://www.codeixer.com/my-account/'>renew your license</a> key to enable updates for ".WPGS_NAME.".</span>";
                }
            }
        }
