...
class WtGtDataFiles
{
	...
	function __construct(){
        if (class_exists('Wt')) Wt::setObject('data_files', $this);
	}
	...
	public function getCountriesForSelect(){
    	...
    	$countries_file_path = WT_GT_PRO_PLUGIN_DIR . '/data/countries.txt';
	...
	public function getCities(){
    	...
    	$cities_file_path = WT_GT_PRO_PLUGIN_DIR . '/data/cities.txt';
    ...
    public function refreshCountriesList(){
        $countries_file_path = WT_GT_PRO_PLUGIN_DIR . '/data/countries.txt';
        $countries_file = fopen($countries_file_path, 'r');
        ...
        fgets($countries_file); // Заголовки
        ...
        while(!feof($countries_file))
        {
            ...
            $country_alpha2 = $arRecord[3];
            $country_name = $arRecord[0];		// Меняем кодировку с windows-1251 на UTF-8 и сохраняем
            $this->countries_list[$country_alpha2] = $country_name;
        }
    }

    public function getCountryName($alpha2){
	    if (empty($this->countries_list)) $this->refreshCountriesList();

	    return $this->countries_list[$alpha2];
    }
}
